...
for p in data['products']:
    if "GIFT SET" in p['title'].upper() or "3 PCS" in p['title'].upper() or "3 PIECES" in p['title'].upper():
        if len(p['images']) > 0:
            # For gift sets, use the very first image because the first image is always the full set with the box!
...
for title, new_img in gift_sets_map.items():
    ...
    if re.search(pattern, code, flags=re.IGNORECASE):
        ...
        updated += 1
        print("Restored Gift Set:", clean_title)

with open(r'c:\web\khadlaj-perfumes (1).jsx', 'w', encoding='utf-8') as f:
    f.write(code)

print(f"Updated {updated} gift sets to use their proper Gift Set images.")
