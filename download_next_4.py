...
url = "https://khadlaj-perfumes.com/products.json?limit=16"
req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
with urllib.request.urlopen(req) as response:
    data = json.loads(response.read().decode())

products = data.get('products', [])[12:16] # Get the next 4 products
...
for idx, p in enumerate(products):
    ...
    prod_str = f'  {{ id: 400{idx}, name: "{name}", col: "Perfume Spray", price: {price}, size: "100ml EDP", badge: "{badge}", gender: "{gender}", notes: ["Vanilla", "Amber", "Oud"], img: "{img}" }},'
